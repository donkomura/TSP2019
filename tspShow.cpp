...
void showTour(int *tou, int wai, int color) {
  ...
  XFillRectangle(dis, pm, gcb, 0, 0, winSize + winMarg * 2,
                 winSize + winMarg * 2);
  ...
  if (color < 0 || toCNum <= color)
    color = 0;
  XSetForeground(dis, gc, toColor[color]);

  for (i = 0; i < n - 1; i++) {
    XDrawLine(dis, pm, gc, icity[tou[i] * 2], icity[tou[i] * 2 + 1],
              icity[tou[i + 1] * 2], icity[tou[i + 1] * 2 + 1]);
  }
  XDrawLine(dis, pm, gc, icity[tou[n - 1] * 2], icity[tou[n - 1] * 2 + 1],
            icity[tou[0] * 2], icity[tou[0] * 2 + 1]);

  XCopyArea(dis, pm, win, gc, 0, 0, winSize + winMarg * 2,
            winSize + winMarg * 2, 0, 0);
  XFlush(dis);
...
void showNTour(int *next, int start, int wai, int color) {
  ...
  for (i = next[start];; i = next[i]) {
    XDrawLine(dis, pm, gc, icity[i * 2], icity[i * 2 + 1], icity[j * 2],
              icity[j * 2 + 1]);
    if (i == start)
      break;
    j = i;
...
int main(int argc, char *argv[]) {
  ...
  if (rndm == 0 || fflag == 1) {
    ...
    while (fgets(buf, BUFSIZE, fp) != NULL) {
      ct = buf[0];
      if (ct == 'N') {
        if (strncmp("NODE_CORD_SECTION", buf, 17) == 0)
          break;
        if (strncmp("NODE_COORD_SECTION", buf, 18) == 0)
          break;
    ...
    for (i = 0; i < n; i++) {
      for (j = 0; j < 2; j++) {
        city[(i + zur) % n][j] =
            (float)((double)rand() / ((double)RAND_MAX / rndRange));
      }
    }
  }
  //;
  ...
  if (gActive != 0) {
    ...
    for (i = 0; i < n; i++) {
      icity[i * 2] =
          (int)((city[i][0] - cenx) * winSize / wid) + winMarg + winSize / 2;
      icity[i * 2 + 1] =
          -(int)((city[i][1] - ceny) * winSize / hei) + winMarg + winSize / 2;
    }
    for (i = 0; i < n; i++) {
      tour[i] = i;
    }
    ...
    win = XCreateSimpleWindow(dis, RootWindow(dis, 0), 50, 50,
                              winSize + winMarg * 2, winSize + winMarg * 2, 3,
                              ctColor, bgColor);
    pm = XCreatePixmap(dis, win, winSize + winMarg * 2, winSize + winMarg * 2,
                       DefaultDepth(dis, 0));

    XSetStandardProperties(dis, win, "tspShow", "tspShow", None, argv, argc,
                           NULL);
